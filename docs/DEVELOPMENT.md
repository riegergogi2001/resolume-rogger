...
- **Five pages** with tabs:
  ...
  - **DJ Intro** — a 24-slot clip grid built dynamically from the live
    ...
    **cross-checks the group**: if the name plate says BRAVO at column 3 but
...
- **Combos** — any pad button can act as a modifier for another: hold it
  ...
  than A alone). Badges show combos as `RT+A`. The shipped layout has **no
  combos** (the 2.2.1–2.2.3 LB/RT combo layout was rolled back in 2.2.5):
  ...
  Combos are the operator's to add. Set a modifier in the editor's
...
The bundled show config (`configs/show.json`, a real show with its
network address and DJ-page names stripped) seeds the user
