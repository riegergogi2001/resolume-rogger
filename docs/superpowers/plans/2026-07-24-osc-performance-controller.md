...
- [ ] `.gitignore`: `node_modules/`, `dist/`, `config.dev.json`, `.idea/`, `*.log`, `test-artifacts/`
- [ ] `package.json`: name `rogger`, `"main": "src/main/main.js"`, scripts:
  `"start": "electron ."`, `"test": "node --test test/"`,
  `"dist:win": "electron-builder --win --x64"`, plus `build` config (Task 10 fills details).
- [ ] `npm install --save-dev electron electron-builder`
- [ ] Commit: `chore: scaffold electron project + design/plan docs`
...
- [ ] Write failing tests → run `npm test` (fails: module missing)
- [ ] Implement encode (4-byte padded strings, `,`+typetags, big-endian i/f)
- [ ] Implement decode incl. `#bundle` recursion
- [ ] `npm test` green → commit `feat: OSC 1.0 codec`
...
- [ ] Failing tests → implement → green → commit `feat: config store with defaults + atomic save`
...
- [ ] Failing tests → implement → green → commit `feat: OSC engine (send/receive/learn/status/test)`
...
- [ ] Implement all three files; manual check `npm start` boots on macOS (blank page OK until Task 6); commit `feat: electron shell + preload bridge`
...
- [ ] Renders in plain browser with mock bridge; commit `feat: renderer shell + theme`
...
- [ ] All wired to state + bridge; manual browser check; commit `feat: performance surface controls`
...
- [ ] Commit `feat: settings, button editor, OSC library, learn mode`
...
- [ ] All assertions pass; commit `test: playwright UI verification`
...
- [ ] Run `npm run dist:win` on macOS (electron-builder cross-packages unsigned portable exe). If toolchain blocks, document exact Windows-side one-liner instead — but attempt first.
- [ ] `README.md`: what it is, Resolume OSC setup (enable OSC in/out, ports 7000/7001, point output at Ally's IP), dev commands, build commands, config location.
- [ ] Final commit `feat: windows packaging + docs`, mark plan checkboxes.
